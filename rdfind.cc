...
int report(const std::string &path, const std::string &name, int depth) {
	...
	RDDEBUG(
			"report("<<path.c_str()<<","<<name.c_str()<<","<<depth<<")" <<std::endl);
...
void usage() {
	...
	cout << endl;
	cout << endl
			<< "rdfind is written by Paul Dreik 2006. License: GPL v2 or later (at your option)."
			<< endl;
...
int main(int narg, char *argv[]) {
		...
		if (arg.at(0) == '-') {
			if (arg == "-makesymlinks" && n < (narg - 1)) {
				...
				if (nextarg == "md5")
					...
					cerr << "expected md5 or sha1, not \"" << nextarg << "\""
							<< endl;
					return -1;
				}
				...
				if (nextarg == "1ms")
					...
					cerr
							<< "sorry, can only understand a few sleep values for now."
							<< endl;
					return -1;
				}
	...
	type[3] =
			(usesha1 ? Fileinfo::CREATE_SHA1_CHECKSUM : Fileinfo::NOT_DEFINED);
