...
public:
	...
	Fileinfo(const std::string& name, int priority = 0) :
			m_magicnumber(771114), m_filename(name), m_delete(false), m_duptype(
					DUPTYPE_UNKNOWN), m_priority(priority), m_identity(0), m_depth(
					0) {
	...
	static int identity(const Fileinfo &A) {
		return A.identity();
	}
	...
	static bool static_deleteflag(const Fileinfo &A) {
		return A.deleteflag();
	}

	//returns the size
	filesizetype size() const {
		return m_info.stat_size;
	}

	//returns true if A has size zero
	static bool isempty(const Fileinfo &A) {
		return A.size() == 0;
	}

	//returns the inode number
	unsigned long inode() const {
		return m_info.stat_ino;
	}

	//returns the device
	unsigned long device() const {
		return m_info.stat_dev;
	}
	...
	void displaybytes() {
		cout << "bytes are \"" << m_somebytes << "\"" << endl;
	}

	//get a specific byte of the buffer
	char getreadbyte(int n) const {
		return m_somebytes[n];
	}
	...
	static bool compareonsizeandfirstbytes(const Fileinfo &a,
			const Fileinfo &b);
